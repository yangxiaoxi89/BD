...
gc()
# 
DotPlot(BD_seu, features = unique(c("FCGR3B","CSF3R","CXCR2","CXCL8","S100A8","S100A9","CAMP","LTF")),group.by = "seurat_clusters")+RotatedAxis()+
  scale_x_discrete("")+scale_y_discrete("")
# 
DotPlot(BD_seu, features = unique(c("FCGR3B","CSF3R","CXCR2","CXCL8","S100A8","S100A9","CAMP","LTF","LCN2")),group.by = "seurat_clusters")+RotatedAxis()+
  scale_x_discrete("")+scale_y_discrete("")
DotPlot(BD_seu, features = unique(c("FCGR3B","CSF3R","CXCR2","CXCL8","S100A8","S100A9","CAMP","LTF","LCN2",
                                    "CD14","CD163","SIGLEC8","CLC","CPA3","MS4A2","CD34")),group.by = "seurat_clusters")+RotatedAxis()+
  scale_x_discrete("")+scale_y_discrete("")
DotPlot(BD_seu, features = unique(c("FCGR3B","CSF3R","CXCR2","S100A8","S100A9","CAMP","LTF","LCN2",
                                    "CD163","CD3D","CD79A","SIGLEC8","CPA3","MS4A2","PF4",
                                    "ITGAM","SELL","CD14","JUN","IER3","EGR1")),group.by = "celltype")+RotatedAxis()+
  scale_x_discrete("")+scale_y_discrete("")
...
DimPlot(BD_seu,split.by = "orig.ident",ncol = 2, repel = T, raster = F,label = T)
# 
group_table = as.data.frame(table(BD_seu@meta.data$celltype, BD_seu@meta.data$orig.ident))
names(group_table) = c("celltype","orig.ident","CellNumber")
plot_group = ggplot(BD_seu@meta.data,aes(x = celltype, fill = orig.ident)) +
  geom_bar(position = "fill") +
...
data = GetAssayData(BD_seu, assay = "RNA", slot = "data")
ids = unique(meta$orig.ident)

#
avg = matrix(NA, nrow = nrow(data), ncol = length(ids))
colnames(avg) = ids
rownames(avg) = rownames(data)
# 
for (id in ids)
{
  cells = rownames(meta)[meta$orig.ident == id]
  avg[, id] = rowMeans(data[, cells, drop = F])
}
# spearman correlation
cor_matrix = cor(avg, method = "spearman")
...
################## activation marker #####################
activation_markers = c("FOS", "DUSP1",
  "JUN", "EGR1", "ATF3", "IER3",  
  "HSPA1A", "HSPA1B", "DNAJB1", 
  "ITGAM"
)

DotPlot(BD_seu, features = activation_markers, group.by = "celltype") +
  RotatedAxis() +
  labs(x = "Activation markers", y = "Subcluster")
